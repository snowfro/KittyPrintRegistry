...
/**************
 * A registry of provably authentic physical prints of Kitties from the Dapper Labs
 * CryptoKitties generative art project (https://www.cryptokitties.co).
 * In order to purchase a print you must be able to prove ownership of the Kitty.
 * This is done by sending payment for the print from the account address that
 * holds the Kitty itself. There are three types of purchases. One is a print of your Kitty
 * accompanied by an NFC that contains the details of your transaction. You may also
 * purchase the NFC by itself if you want to print your own Kitty. There is a Misc purchase
...
interface KittyInterface {
    ...
    function ownerOf(uint256 _kittyId) external view returns (address);
}

// File: contracts/KittyPrintRegistry.sol

pragma solidity ^0.5.0;

contract KittyPrintRegistry is CustomERC721Metadata, WhitelistedRole {
    using SafeMath for uint256;
    ...
    mapping(uint256 => uint256) public tokenIdToKittyId;
    ...
    function purchasePrint(uint256 _kittyId, string memory _contactMethod) public payable returns (uint256 _tokenId) {
        ...
        require(msg.sender == kittyContract.ownerOf(_kittyId), "You must own kitty");

        uint256 tokenId = _mintToken(msg.sender);

        tokenIdToKittyId[tokenId] = _kittyId;
    ...
    function purchaseNFCOnly(uint256 _kittyId, string memory _contactMethod) public payable returns (uint256 _tokenId) {
    ...
    function purchaseMisc(uint256 _kittyId, string memory _contactMethod) public payable returns (uint256 _tokenId) {
    ...
    function mint(uint256 _kittyId, string memory _nfc) public onlyWhitelisted returns (uint256 _tokenId) {
        uint256 tokenId = _mintToken(kittyContract.ownerOf(_kittyId));
        tokenIdToKittyId[tokenId] = _kittyId;
    ...
    function updateTokenKittyId(uint256 _tokenId, uint256 _kittyId) public onlyWhitelisted returns (bool) {
        require(_exists(_tokenId), "Token must exist");

        tokenIdToKittyId[_tokenId] = _kittyId;
        return true;
    }
